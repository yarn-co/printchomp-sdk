...
PrintChompSDK.prototype.getOffers = function (offerId, productName, exclusiveOffer, cb) {
  ...
  if (offerId) {
    ...
    Hoek.assert(offerId == parseInt(offerId), new Error('The received offer id is not an integer'));
...
PrintChompSDK.prototype.getFiles = function (fileId, orderId, cb) {
  ...
  // ensure that a parameter was received
  Hoek.assert(fileId || orderId, new Error('Either fileId or orderId must be set'));
  
  // ensure that what was passed was an integer
  if (fileId) {
    
    Hoek.assert(fileId == parseInt(fileId), new Error('The received fileId is not an integer.'));
    
    filesUrl = 'files/'+parseInt(fileId);
    
  } else {
    
    Hoek.assert(orderId == parseInt(orderId), new Error('The received orderId is not an integer.'));
    
    filesUrl = 'orders/'+parseInt(orderId)+'/files';
  }
